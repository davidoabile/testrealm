...
export class Service extends PickNScan {
  ...
  async getPdf() {
    ...
    console.log(r)
    //const url = `/fastlabel/getlabelpdf/${this.config.charge_to}?ConsignmentID=${this.consignment['ConsignmentID']}`;
    const url = '/fastlabel/GetConsignmentLabelsPdf?UserID=4397&ConsignmentID=90209516&'
    console.log(this.getUrl(url))
    const res = await this.download(this.getUrl(url), this.pdfFile)
    console.log(res)
  }
